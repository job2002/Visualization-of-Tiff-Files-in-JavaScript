...
L.LeafletGeotiff = L.ImageOverlay.extend({
    ...
    _getData: function(options) {
        ...
        request.withCredentials = options.sendHttpReqCredentials
    ...
    _reset: function () {
        if (this.hasOwnProperty('_map')) {
            if (this._rasterBounds) {
                let topLeft = this._map.latLngToLayerPoint(this._map.getBounds().getNorthWest()),
    ...
    transform: function(rasterImageData, args) {
        ...
        try {
            ...
            let ptx = document.createElement('canvas');
            ptx.globalAlpha = (this.options.opacity);
            console.log('geotiff-layer-leaflet.js transform opacity', this.options.opacity);
            imageData = ptx.createImageData(args.plotWidth, args.plotHeight);
        }
